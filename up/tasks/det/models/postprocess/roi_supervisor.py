...
@ROI_SUPERVISOR_REGISTRY.register('lttss')
class LTTSSSupervisor(object):
    '''
    Compuate targets for Long-tailed Training Sample Selection (LTTSS)
    '''

    def __init__(self, k=9, mode=None, param=None, statis=False, rand_factors=None):
        self.k = k
        self.mode = mode
        self.param = param
        rc = np.load('rc.npy', allow_pickle=True).item()
        rc = torch.tensor(list(rc.values())[:1203]).cuda()
        self.rc = rc
        self.statis = statis
        if statis == True:
            self.num_anchors = np.zeros(1203, dtype=np.int64)
        self.rand_factors = rand_factors
    ...
    def get_targets(self, mlvl_anchors, input, mlvl_preds=None):
        ...
        for b_ix in range(B):
            ...
            if gt.shape[0] == 0:
                ...
                candidate_idxs = []
                if self.mode == 'rc':
                    r_cg = self.rc.index_select(dim=0, index=labels.long() - 1)
                    s_cg = 1 + torch.div(self.param[0], (1 + torch.exp((r_cg - self.param[2]) * -self.param[1])))
                    k_cg = self.k * s_cg
                    k_cg_INT = k_cg.int().cpu().numpy().tolist()
                star_idx = 0
                for num in num_anchors_per_level:
                    end_idx = star_idx + num
                    distances_per_level = distances[star_idx:end_idx]
                    if self.mode == 'rc':
                        topk = min(self.k * 2, num)
                    else:
                        topk = min(self.k, num)
                    _, topk_idxs_per_level = distances_per_level.topk(topk, dim=0, largest=False)
                    candidate_idxs.append(topk_idxs_per_level + star_idx)
                ...
                if self.mode == 'rc':
                    iou_mean_per_gt = []
                    iou_std_per_gt = []
                    for i in range(0, candidate_ious.shape[1]):
                        candidate_iou = candidate_ious[:, i]
                        candidate_iou = torch.cat((candidate_iou[self.k * 2 * 0:self.k * 2 * 0 + k_cg_INT[i]],
                                                   candidate_iou[self.k * 2 * 1:self.k * 2 * 1 + k_cg_INT[i]],
                                                   candidate_iou[self.k * 2 * 2:self.k * 2 * 2 + k_cg_INT[i]],
                                                   candidate_iou[self.k * 2 * 3:self.k * 2 * 3 + k_cg_INT[i]],
                                                   candidate_iou[self.k * 2 * 4:self.k * 2 * 4 + k_cg_INT[i]]))
                        iou_mean_per_gt.append(candidate_iou.mean())
                        iou_std_per_gt.append(candidate_iou.std())
                    iou_mean_per_gt = torch.tensor(iou_mean_per_gt).cuda()
                    iou_std_per_gt = torch.tensor(iou_std_per_gt).cuda()
                else:
                    iou_mean_per_gt = candidate_ious.mean(0)
                    iou_std_per_gt = candidate_ious.std(0)
                iou_thresh_per_gt = iou_mean_per_gt + iou_std_per_gt
                ...
                if self.mode == 'rc':
                    iou_thresh_per_gt = torch.div(iou_thresh_per_gt, s_cg)
                elif self.mode == 'rand':
                    iou_thresh_per_gt = torch.div(iou_thresh_per_gt, self.rand_factors)
                is_pos = candidate_ious >= iou_thresh_per_gt[None, :]
                if self.mode == 'rc':
                    is_true_candidate = torch.zeros_like(is_pos, dtype=torch.bool).cuda()
                    for i in range(0, is_true_candidate.shape[1]):
                        is_true_candidate[self.k * 2 * 0:self.k * 2 * 0 + k_cg_INT[i], i] = True
                        is_true_candidate[self.k * 2 * 1:self.k * 2 * 1 + k_cg_INT[i], i] = True
                        is_true_candidate[self.k * 2 * 2:self.k * 2 * 2 + k_cg_INT[i], i] = True
                        is_true_candidate[self.k * 2 * 3:self.k * 2 * 3 + k_cg_INT[i], i] = True
                        is_true_candidate[self.k * 2 * 4:self.k * 2 * 4 + k_cg_INT[i], i] = True
                    is_pos = is_pos & is_true_candidate
                ...
                if self.statis == True:
                    ...
                    for unique_label in unique_labels[1:]:
                        label_anchors = len(torch.nonzero(labels == unique_label))
                        self.num_anchors[int(unique_label.item()-1)] = self.num_anchors[int(unique_label.item()-1)] + label_anchors

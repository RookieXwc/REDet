...
def get_transform(src: np.ndarray, restricts: dict, config: InstaBoostConfig):
...
def __random_transform(restricts: dict, config: InstaBoostConfig):
    t = dict()
    t['s'] = random.uniform(*config.scale)
    max_x = abs(restricts['bbox_w'] // config.dx)
    t['tx'] = random.randint(-max_x, max_x)
    max_y = abs(restricts['bbox_h'] // config.dy)

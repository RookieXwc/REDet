...
class TrainEvalLogger(Hook):
    ...
    def __init__(self, runner, freq=1, logdir='log', skip_first_k=1):
        """
        Arguments:
            - runner (:obj:`Runner`): used as to accecss other variables
...
@HOOK_REGISTRY.register('retinanet_noaug')
class RetinaNoaug(Hook):
    def __init__(self, runner, no_aug_epoch=15, max_epoch=300, transformer=[]):
        super(RetinaNoaug, self).__init__(runner)
...
@HOOK_REGISTRY.register('statis_anchor')
class StatisticsAnchor(Hook):
    def __init__(self, runner, savename):
        super(StatisticsAnchor, self).__init__(runner)
        self.savename = savename

    def before_eval_forward(self, cur_iter, input):
        runner = self.runner_ref()
        num_anchors = runner.model.post_process.supervisor.num_anchors
        np.save(self.savename + '.npy', num_anchors)
